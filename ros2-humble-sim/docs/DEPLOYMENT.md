...
1. Use WSL2 + Ubuntu 22.04 for the current local ROS 2 development environment.
2. Keep Docker files only as a future server/team-image path.

The recommended ROS distribution is ROS 2 Humble on Ubuntu 22.04. Humble is a stable LTS choice for robot simulation, Gazebo, MoveIt 2, and ros2_control.
...
Then install Docker Desktop and enable WSL integration for the Ubuntu 22.04 distribution.
...
docker/
  ...
  run_wsl_gui.sh
scripts/
  setup_wsl_ros2_humble.sh
```

## Suggested execution order

1. Enable WSL2 and install Ubuntu 22.04.
2. Run `scripts/setup_wsl_ros2_humble.sh` inside Ubuntu.
