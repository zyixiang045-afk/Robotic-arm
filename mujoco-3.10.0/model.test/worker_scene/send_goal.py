...
MAP_ORIGIN_WORLD = (-8.5, 0.0)   # 世界系 -> map 系的偏移


class GoalSender(Node):
    def __init__(self, x, y, frame, wait):
        ...
        gx, gy = x, y
        if frame == "world":
            gx = x - MAP_ORIGIN_WORLD[0]
            gy = y - MAP_ORIGIN_WORLD[1]
        self.goal = (gx, gy)
...
def main():
    ...
    rclpy.init()
    node = GoalSender(args.x, args.y, args.frame, args.wait)
    node.send()
    if args.wait:
        node.spin_wait()
    node.destroy_node()
