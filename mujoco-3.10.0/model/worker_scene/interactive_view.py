...
XML = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scene_with_robot.xml")
...
def run_windowed(show_cams=True):
    ...
    state = {"paused": True, "wave": False}
    print("就绪: viewer 窗口鼠标交互" + (
        " | cv2 窗口: 空格=暂停 W=摆臂 R=复位 ESC/q=退出" if show_cams
        else "（--no-cams：物理默认暂停，用 viewer 自带控件播放）"))
