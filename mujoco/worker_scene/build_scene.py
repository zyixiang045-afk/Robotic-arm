...
# ---------------------------------------------------------------------------
def rail_shuttle(name, x, y0, amp, color, mass=6.0, stiffness=22.0):
    """沿 y 轴往复的料车：静态导轨 + 滑块。滑块用 slide 关节，弹簧平衡点在 +amp，
    起始在 0（即 y=y0），故在 [y0, y0+2*amp] 之间往复。悬空 3 mm 于轨上，避免摩擦损耗。
    周期 T ≈ 2π·sqrt(m/k)。"""
    rail_len = amp + 0.35
    return f'''    <!-- 导轨（静态） -->
    <body name="{name}_rail" pos="{x} {round(y0+amp,3)} 0.03">
      <geom type="box" size="0.06 {round(rail_len,3)} 0.03" material="metal"/>
    </body>
    <!-- 料车（沿 y 往复） -->
    <body name="{name}" pos="{x} {y0} 0.16">
      <joint name="{name}_j" type="slide" axis="0 1 0" damping="0"
             stiffness="{stiffness}" springref="{amp}"/>
      <geom type="box" size="0.22 0.22 0.10" rgba="{color}" mass="{mass}"/>
...
def build_xml(rng):
    ...
    # 规则运动的物体（仅两个：一个平移料车 + 一个旋转扫掠杆） ----------------
    movers = [
        rail_shuttle("shuttle_a", -3.5, -0.9, 0.9, "0.90 0.45 0.15 1", mass=6.0, stiffness=22.0),
        sweep_arm("sweep_1", -6.0, 0.0, 1.6, "0.85 0.75 0.15 1", stiffness=9.0),
    ]
...
{blue}
    ...
    <body name="worker" pos="{WORKER_X} 0 0">
      <geom name="w_leg_l" type="capsule" fromto="0  0.11 0.04 0  0.11 0.46" size="0.065" material="worker"/>
      <geom name="w_leg_r" type="capsule" fromto="0 -0.11 0.04 0 -0.11 0.46" size="0.065" material="worker"/>
...
{movers_xml}
  ...
  <contact>
    ...
    <exclude body1="sweep_1"   body2="sweep_1_post"/>
  </contact>
</mujoco>
'''
    return head + body
